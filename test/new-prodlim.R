"prodlim" <- function(formula,
  ...
  if (sum(match(c("subset","na.action"),names(m),nomatch = 0))>0)
    stop("Arguments subset and na.action are not supported by prodlim version 1.3.6 and higher")
  # {{{ find the data
  formList <- readFormula(formula,
                          specials=c("strata", "NN","cluster"),
                          alias=list("dummy"="strata","factor"="strata"),
                          unspecified="unspecified")
  ## if (is.environment(data))
  #  FIX for people and functions who use `Surv' instead of `Hist'
  ## if (substr(formList$Response,0,5)=="Surv(")
  if (formula.names[2]=="Surv"){
    formList$Response <- sub("^Surv","Hist",formList$Response)
  }
  ## response <- model.response(model.frame(formula=update(.~1,formula),data=data))
  if (NROW(response) == 0) stop("No (non-missing) observations")
  ...
  if (is.null(formList$strata)){
    ...
    discrete.matrix <- model.frame(formula=stats::formula(formList$strata$formula),data=data)
  }
  if (is.null(formList$NN)){
    continuous.matrix <- NULL
  } else{  
    continuous.matrix <- model.frame(formula=stats::formula(formList$NN$formula),data=data)
  }
  if (is.null(formList$cluster)){
    cluster.matrix <- NULL
  } else{  
    cluster.matrix <- model.frame(formula=stats::formula(formList$cluster$formula),data=data)
  }
  continuous.predictors <- all.vars(formula(formList$NN$formula))
  ...
  if (!is.null(formList$unspecified)){
    rest <- model.frame(stats::formula(formList$unspecified$formula),data=data)
...
# }}}
  ...
  if (clustered)
    clustervar <- all.vars(formula(formList$cluster$formula))
  else
    clustervar <- NULL
  if (clustered){
